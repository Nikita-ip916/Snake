...
int main()
{
    while (1) {
        ...
        string plNumber = "1";
        cout << "Введите кол-во игроков или выйдите из игры:\n";
        do {
            cin >> plNumber;
        } while (plNumber != "1" && plNumber != "2" && plNumber != "one"
                 && plNumber != "two" && plNumber != "один" && plNumber != "два"
                 && plNumber != "exit" && plNumber != "выход");
        if (plNumber == "exit" || plNumber == "выход") {
            break;
        } else if (plNumber == "one" || plNumber == "один") {
            plNumber = "1";
        ...
        while (window.isOpen()) {
            ...
            while (window.isOpen()) {
                ...
                if (plNumber == "2") {
                    ...
                    if (p1.winner) {
                        gameWon.setPosition(
                                view.getCenter().x - 440,
                                view.getCenter().y - 256);
                        window.draw(gameWon);
                    } else if (p1.life) {
                        bonusText.setString(
                                L"Действие бонуса: " + bonus[1].str());
                        bonus[1].str("");
                        bonusText.setPosition(
                                view.getCenter().x - 406,
                                view.getCenter().y - 406);
